...
### 2.4 设置模式指令 (SettingActivity)

#### 进入设置模式
```
指令格式: AA BB 04 CC
字节数组: [0xAA, 0xBB, 0x04, 0xCC]
发送时机: SettingActivity.onCreate()
重试机制: 800ms超时，最多重试1次
状态验证: 收到 "state set\r\n" 后停止重试
```
...
指令格式: AA BB FF CC
字节数组: [0xAA, 0xBB, 0xFF, 0xCC]
发送时机: SettingActivity.onDestroy()
重试机制: 无（直接发送，不等待响应）
```
...
字节数组: [0xAA, 0xBB, motorIndex, settingType, settingValue, 0xCC]
数据长度: 6字节

电机序号映射: (与手动控制相同)
  0x01: 拇指 (range_slider_thumb)
  0x02: 食指 (range_slider_index)
  0x03: 中指 (range_slider_middle)
  0x04: 无名指 (range_slider_ring)
  0x05: 小指 (range_slider_pinky)
  0x06: 手肘 (range_slider_elbow)

设置类型:
  0x01: 下限值 (lowerValue)
  0x02: 上限值 (upperValue)

设置值范围: 根据电机类型而定（与手动控制档位范围相同）

发送时机: 用户松开RangeSlider时 (onStopTrackingTouch)
触发条件: 下限或上限值发生变化
数据持久化: 保存到SharedPreferences，区分左右手
重试机制: 无（直接发送）
```

**示例**:
- 设置拇指下限为5: `AA BB 01 01 05 CC`
- 设置拇指上限为25: `AA BB 01 02 19 CC`
- 设置手肘下限为3: `AA BB 06 01 03 CC`

**工作机制**:
- 使用RangeSlider设置每个电机的上下限范围
- 记录上一次的上下限值，只有变化时才发送
- 分别检测下限和上限的变化，分别发送指令
- 设置值持久化到SharedPreferences，键名格式:
  - `"[true/false]_[sliderId]_lowerValue"`
  - `"[true/false]_[sliderId]_upperValue"`
- 根据Select.select_hand加载对应的保存值
